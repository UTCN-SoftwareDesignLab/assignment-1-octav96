...
public class RightsRolesRepositoryMySQL implements RightsRolesRepository {
    ...
    @Override
    public Notification<Role> findRoleByTitle(String role) {
        Statement statement;
        try {
            statement = connection.createStatement();
            String fetchRoleSql = "Select * from " + ROLE + " where `role`=\'" + role + "\'";
            Notification<Role> notification = new Notification<>();
            ...
            if(role.equalsIgnoreCase("administrator") || role.equalsIgnoreCase("employee")){
                ...
                notification.setResult(new Role(roleId, roleTitle, null));

                return notification;
            }else{
               notification.setResult(null);
               notification.addError("Role search did not work!");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    @Override
    public void deleteRolesFromUser(Long id){
        try {
            Statement statement = connection.createStatement();
    ...
    @Override
    public List<Long> findRoleIdsForUser(Long userId) {
        try {
